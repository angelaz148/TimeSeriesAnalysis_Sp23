...
```{r}
biomass_prod_mean<- mean(data$Total.Biomass.Energy.Production)
biomass_prod_sd<- sd(data$Total.Biomass.Energy.Production)
renew_prod_mean<- mean(data$Total.Renewable.Energy.Production)
renew_prod_sd<- sd(data$Total.Renewable.Energy.Production)
hydro_prod_mean<- mean(data$Hydroelectric.Power.Consumption)
hydro_prod_sd<- sd(data$Hydroelectric.Power.Consumption)

#might how to output them
```
...
```{r}
for(i in 1:(ncol(data)-1)){
  Pacf(ts_inflow_data[,i], lag.max=40,main=name[i+1], ylim=c(-1,1))
}
