...
MPE = sum((actual - forecast)/actual)/n*100 #correct, checked with luana's 
MAPE = (sum((abs(actual - forecast))/actual*100))/n
MAPE2 = sum(abs(actual - forecast)/actual)/n*100 #gets the same as MAPE 

manual_scores<- as.data.frame(cbind(ME, RMSE, RMSE, MAE, MPE, MAPE))
