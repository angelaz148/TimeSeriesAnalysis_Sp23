...
```{r}
ts_inflow_data<- ts(inflow_data[,(2:16)], frequency = 12, start = c(1931,1))
head(ts_inflow_data)
```
...
#want to know about the seasonality and the correlation 
```
...
ggplot(inflow_data, aes(x=my_date, y=inflow_data[,iHP+1])) +geom_line(color='blue')+
         geom_smooth(method="lm", color ='red')

```

use coefficients from lm to subtract from the dataset 
```{r}
...
seas_mean_model<- lm(inflow_data[,(iHP+1)] ~ dummies)
summary(seas_mean_model)

#there are negative values bc they are relative to december, which is the coefficient 

betas0<- seas_mean_model$coefficients[1]
betas1<- seas_mean_model$coefficients[2:12]

inflow_seas_comp<- array(0,nobs)
for(i in 1:nobs){
  inflow_seas_comp[i]<- betas0 + betas1 %*% dummies[i,]
}
...
#if this tells you there is a unit root, then theres no use running a linear trend 
#if it returns stationarity, then still have to run mann kendall test 
