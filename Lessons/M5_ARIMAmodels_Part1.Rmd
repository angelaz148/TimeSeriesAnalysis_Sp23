...
#AR model. slow decay on acf, and cut off at lag one at pacf 
#very strong AR, might have to do differencing to do ARIMA 
```
...
#the waves in the acf are gone bc we removed the seasonality
...
#in the first pacf, can see the spikes at lag 12 and 24 which are of the seasonality 
...
#greater than 0.05, tells us to accept -> not stationariy and has a unit root 
#saw that: at lag 1, the pacf is at 1 
#so need diffeereencing 
...
#the lag 1 is no longer close to 1 
#wont find unit root anymore, now have a stationary process
#which we need for arima model 
...
#lag 1 = 1 is gone here too 
#now ready to fit arima model 
...
#she usually allows the drift, allows mean to be different from zero -> more generous 
print(Model_111)
...
#use AICc if we have a small sample set (rather than AIC)
...
#have some points outside of blue lines, but no red flags
#ideally have no points outside the blue lines 
...
print(compare_aic)
#looks like model_212 is the best bc it has the smallest AIC value 
```
